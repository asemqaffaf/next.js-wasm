...
export default async function Index() {
  ...
  const { default: initWasm, greet, add, fetch_wasm_json, fetch_wasm_map } = await import('../../../wasm-module/pkg/wasm_module');
  ...
  const apiMap = await fetch_wasm_map('https://jsonplaceholder.typicode.com/todos/1').catch((e) => console.error(e));
  ...
  return (
    <div className='flex flex-col justify-center items-center h-screen w-screen'>
      ...
      <h1 className='text-4xl'>{JSON.stringify([...apiMap])}</h1>
    </div>
  );
}
